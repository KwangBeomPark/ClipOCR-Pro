...
- **📐 이미지 리사이즈 및 복사**: 앱 설정값에 따라 캡처 이미지를 크기 조정한 뒤 문서, 이메일, 보고서에 붙여 넣을 수 있습니다.
...
2. 이 저장소를 Clone합니다.
3. `ClipOCR-Pro.ahk` 소스 코드를 필요에 맞게 수정합니다.
4. AutoHotkey 컴파일러, Ahk2Exe 또는 포함된 빌드 스크립트를 사용하여 본인만의 `ClipOCR-Pro.exe`를 패키징합니다.
...
> 매뉴얼 인포그래픽을 추후 추가할 경우, 저장소에 `App01Manual.png` 파일명으로 업로드한 뒤 이 위치에 참조하면 됩니다.
...
ClipOCR-Pro 설정값은 로컬 `config.ini` 파일에 저장됩니다.

팀 배포 시에는 한 명의 관리자가 권장 캡처 크기, 번역 옵션, 이미지 리사이즈 설정, 기타 기본값을 먼저 세팅한 뒤 `config.ini` 파일을 동료에게 공유하면 됩니다.

권장 포터블 배포 구조는 아래와 같습니다.
...
공유된 `config.ini`를 실행 파일과 같은 폴더에 두면 팀원들이 별도 수동 설정 없이 동일한 기준으로 시작할 수 있습니다.
...
- `config.ini`를 팀원에게 배포하기 전에는 API 관련 설정이나 업무 특화 설정이 포함되어 있는지 반드시 검토하세요.
